...
import { SoundEvent } from "../gameassets/gameassets";
 ...
 * - call `blips.print(text: string, options?: { withSound?: boolean, soundPos?: Vector, soundEvent?: SoundEvent })` to show a blip message
 */
class Blips {
    ...
    private static parseColoredText(text: string): {
        text: string;
     ...
     */
    print(text: string, options?: { soundPos?: Vector; soundEvent?: SoundEvent, withSound?: boolean }): void {
        ...
        if (options?.withSound) this.playSound(options?.soundEvent, options?.soundPos);
    }

    /**
     * displays static text at a fixed position for a specified duration
     * @param text the message text, supports color tags like {red}, {green}, etc
     * @param pos screen position {x, y}
     * @param duration how long to display the message in seconds
     */
    printStatic(text: string, pos: { x: number; y: number }, duration: number): void {
        const gameTime = css.GetGameTime();
        const messageId = this.nextMessageId++;
    ...
    update(gameTime: number): void {
        ...
        for (let i = this.staticMessages.length - 1; i >= 0; i--) {
            const msg = this.staticMessages[i];
            const elapsed = gameTime - msg.spawnTime;

            if (elapsed > msg.duration) {
                this.staticMessages.splice(i, 1);
                continue;
            }
            ...
            for (const segment of segments) {
                ...
                xOffset += segment.text.length * 7;
            }
        }
    }

    private playSound(soundEvent: SoundEvent | undefined, pos: Vector | undefined): void {
        if (!pos) pos = { x: 0, y: 0, z: 0 };
        if (!soundEvent) soundEvent = { path: "Alert.WarmupTimeoutBeep" };
